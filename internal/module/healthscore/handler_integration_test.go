...
func (integrationRepo) GetSnapshot(ctx context.Context, userID string) (*Snapshot, error) {
	return &Snapshot{
		TotalAssets:    "1000.00",
		TotalDebts:     "200.00",
		RecentIncoming: "1500.00",
		RecentOutgoing: "900.00",
	}, nil
}
...
func TestFinancialHealthEndpoint(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/financial-health", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	res := httptest.NewRecorder()
	router.ServeHTTP(res, req)
	if res.Code != http.StatusOK {
		t.Fatalf("expected 200 from /financial-health, got %d", res.Code)
	}
}
