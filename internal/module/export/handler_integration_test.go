...
func (integrationRepo) ListMutations(ctx context.Context, userID string, filter MutationFilter) ([]MutationRow, error) {
	return []MutationRow{
		{
			ID:            "mutation-1",
			WalletName:    "Cash",
			...
			Description:   "salary",
			RelatedToDebt: false,
			HappenedAt:    time.Date(2026, 4, 1, 10, 0, 0, 0, time.UTC),
...
func TestExportMutationsCSV(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/exports/mutations.csv?from=2026-04-01&to=2026-04-30", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	res := httptest.NewRecorder()
	router.ServeHTTP(res, req)
	if res.Code != http.StatusOK {
		t.Fatalf("expected 200 from export endpoint, got %d", res.Code)
	}
	if got := res.Header().Get("Content-Type"); !strings.Contains(got, "text/csv") {
		t.Fatalf("expected csv content type, got %q", got)
	}
	if !strings.Contains(res.Body.String(), "wallet_name,category_name") {
		t.Fatalf("expected csv header, got %s", res.Body.String())
	}
}
