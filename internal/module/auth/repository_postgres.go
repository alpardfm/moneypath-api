...
func (r *PostgresRepository) GetUserByEmail(ctx context.Context, email string) (*User, error) {
	return r.getOne(ctx, `SELECT id, email, username, password_hash, full_name, created_at, updated_at FROM users WHERE email = $1`, email)
}

func (r *PostgresRepository) GetUserByUsername(ctx context.Context, username string) (*User, error) {
	return r.getOne(ctx, `SELECT id, email, username, password_hash, full_name, created_at, updated_at FROM users WHERE username = $1`, username)
}

func (r *PostgresRepository) GetUserByEmailOrUsername(ctx context.Context, value string) (*User, error) {
	return r.getOne(ctx, `SELECT id, email, username, password_hash, full_name, created_at, updated_at FROM users WHERE email = $1 OR username = $1`, value)
}

func (r *PostgresRepository) GetUserByID(ctx context.Context, userID string) (*User, error) {
	return r.getOne(ctx, `SELECT id, email, username, password_hash, full_name, created_at, updated_at FROM users WHERE id = $1`, userID)
}
...
func (r *PostgresRepository) UpdateProfile(ctx context.Context, userID, email, username, fullName string) (*User, error) {
	...
	err := r.pool.QueryRow(ctx, query, userID, email, username, fullName).
		Scan(&user.ID, &user.Email, &user.Username, &user.PasswordHash, &user.FullName, &user.CreatedAt, &user.UpdatedAt)
	if err == nil {
		return user, nil
	}
